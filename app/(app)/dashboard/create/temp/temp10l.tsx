...
export default function TempV10L() {
  const f = useTempForm();
  ...
  return (
    <div className="min-h-[100vh] grid grid-rows-[auto,1fr]">
      ...
      <main className="p-4 sm:p-6 grid gap-4">
        ...
        {/* Bottom-docked dual uploaders (clear buttons) */}
        <div className="fixed left-1/2 -translate-x-1/2 bottom-20 z-30 flex gap-2">
          {(f.needsFile1||f.needsFile2) && <AttachPill icon={<Home className="h-4 w-4" />} label="Base" onPick={f.setFile1} />}
...
function AttachPill({ icon, label, onPick }: { icon: React.ReactNode; label: string; onPick: (f: File | null)=>void }) {
  return (
    <label className="inline-flex items-center gap-2 rounded-full border bg-background px-3 h-10 cursor-pointer text-sm shadow hover:bg-accent">
      {icon}
